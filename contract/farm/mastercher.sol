...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    address constant _trustedForwarder = 0x52F08070af74Ef7D2C3C80a882Ab8332f576a7cC; //TRUSTED FORWARDER
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. Solar to distribute per block. 分配给此池的分配点数量。太阳能分配到每个区块。
        ...
        uint16 depositFeeBP; // Deposit fee in basis points 按基点计算的存款费用
        uint256 harvestInterval; // Harvest interval in seconds 收获间隔（秒）
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        require(block.number >= startBlock, "SunfDistributor: Can not deposit before start");
