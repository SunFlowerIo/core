...
export default function TokenStatsModal({ token }: { token: any }) {
  ...
  tokenInfo = { circulatingSupply: '1500000', burnt: '0', totalSupply: '0', vaults: '0' }

  const price = formatNumberScale(priceData?.[token.symbol.toLowerCase()], true, 2)

  const modalOpen = useModalOpen(token.symbol == 'SUNF1' ? ApplicationModal.SOLAR_STATS : ApplicationModal.MOVR_STATS)
          ...
          <div className="flex flex-row w-full py-4">
            ...
            <div className="flex flex-1 flex-col">
              ...
              <div className="flex items-center justify-between space-x-3 gap-2">
                {token?.address && (
                  <ExternalLink
                    href={'https://shiden.subscan.io/tokens/' + `${token?.address[chainId]}`}
