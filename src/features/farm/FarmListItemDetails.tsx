...
const FarmListItem = ({ farm }) => {
  ...
  return (
    <Transition
      ...
      leaveTo="opacity-0"
    >
      <Disclosure.Panel className="flex flex-col w-full border-t-0 rounded rounded-t-none mybg900" static>
        <div className="grid grid-cols-2 gap-4 p-4">
          <div className="col-span-2 text-center md:col-span-1">
            ...
            <div className="relative flex items-center w-full mb-4">
              <NumericalInput
                className="w-full px-4 py-4 pr-20 rounded mybg800 focus:ring focus:ring-dark-purple"
                value={depositValue}
              ...
              {account && (
                <Button
                  ...
                  color="light-green"
                  size="xs"
                  disabled={farm?.id === '1111'}
                  onClick={() => {
                    if (!balance.equalTo(ZERO)) {
                      if (liquidityToken?.symbol == 'SUNF') {
                        try {
            ...
            {approvalState === ApprovalState.NOT_APPROVED || approvalState === ApprovalState.PENDING ? (
              ...
              <Button
                ...
                variant="outlined"
                color="gradient"
                disabled={pendingTx || !typedDepositValue || balance.lessThan(typedDepositValue) || farm?.id === '1111'}
                onClick={async () => {
                  setPendingTx(true)
                  try {
          ...
          <div className="col-span-2 text-center md:col-span-1">
            ...
            <div className="relative flex items-center w-full mb-4">
              <NumericalInput
                className="w-full px-4 py-4 pr-20 rounded mybg800 focus:ring focus:ring-light-purple"
                value={withdrawValue}
        ...
        {pendingSolar && pendingSolar.greaterThan(ZERO) && (
          <div className="px-4 pb-4">
            ...
            >
              {i18n._(t`Harvest ${formatNumber(pendingSolar.toFixed(18))} SUNF`)}
            </Button>
          </div>
        )}
      </Disclosure.Panel>
