...
import CurrencyLogoWithTitle from "../../components/CurrencyLogo/title";
  ...
  const map = (pool) => {
    ...
    function getTvl(pool) {
      ...
      if (pool.lpToken == SOLAR_ADDRESS[chainId]) {
        ...
        lpPrice = movrPrice
      }else if(pool.lpToken == '0xc946DAf81b08146B1C7A8Da2A851Ddf2B3EAaf85' || pool.lpToken == "0x382bB369d343125BfB2117af9c149795C6C65C50"){
        return Number(pool.totalLp / 10 ** decimals);
      } else {
        lpPrice = 0
      }
        ...
        <DoubleGlowShadow maxWidth={false} opacity={'0.4'}>
          <div className={`grid grid-cols-12 gap-2 min-h-1/2`}>
            <div className={`col-span-12`}>
              <Card className="mybg900 z-4">
                <div className={`grid grid-cols-12 md:space-x-4 space-y-4 md:space-y-0 `}>
                  <div className={`col-span-12 md:col-span-3 space-y-4`}>
                    <div className={`hidden md:block`}>
                      <div className={`col-span-12 md:col-span-4 mybg800 px-6 py-4 rounded`}>
                        <div className="mb-2 text-2xl text-emphesis colorfff">{i18n._(t`SunFlower Vault`)}</div>
                        <div className="mb-4 text-base text-secondary">
                          <p className="color13b">
                            {i18n._(
                              t`SUNF Vault is a set of high incentivized pools. Long term supporters can choose to lock SUNF for a determined period for higher rewards.`
                            )}
                          </p>
                          <p className="mt-2 color13b">
                            {i18n._(
                              t`The participants receive various benefits such as higher rewards according to lock duration, higher allocations in Sunf Launchpad and more.`
                            )}
                          </p>
                        </div>
                        <div className="mb-2 text-2xl text-emphesis colorfff">{i18n._(t`Considerations`)}</div>{' '}
                        <div className="mb-4 text-base text-secondary">
                          <p className="color13b">{i18n._(t`Everytime you stake or claim rewards your lock time renews.`)}</p>
                        </div>
                      </div>
                    </div>
                    <div className={`flex flex-col items-center justify-between px-6 py-6 `}>
                      <div className="flex items-center justify-between py-2 text-emphasis color13b">
                  ...
                  <div className={`col-span-12 md:col-span-9 mybg800  py-4 md:px-6 md:py-4 rounded`}>
