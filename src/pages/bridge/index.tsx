...
export default function Bridge() {
  ...
  const [chainTo, setChainTo] = useState<Chain | null>(chainId == ChainId.SDN ? DEFAULT_CHAIN_FROM : DEFAULT_CHAIN_TO)
  ...
  const anyswapCurrencyContract = useAnyswapTokenContract(
    currency0 && currency0.chainId == ChainId.SDN && tokenToBridge.other.ContractAddress,
    true
  )
  ...
  const handleChainFrom = useCallback(
    (chain: Chain) => {
      ...
      if (changeTo.id !== ChainId.SDN && chain.id !== ChainId.SDN) {
  ...
  const handleChainTo = useCallback(
    (chain: Chain) => {
      ...
      if (changeFrom.id !== ChainId.SDN && chain.id !== ChainId.SDN) {
  ...
  const bridgeToken = async () => {
    ...
    const depositAddress = currency0.chainId == ChainId.SDN ? token.ContractAddress : token.DepositAddress

    const amountToBridge = ethers.utils.parseUnits(currencyAmount, token.Decimals)
    setPendingTx(true)

    try {
      if (currency0.chainId == ChainId.SDN) {
  ...
  return (
    <>
      ...
      <Head>
        <title>{i18n._(t`Bridge`)} | SunFlower</title>
